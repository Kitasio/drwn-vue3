<template>
  <stock-nav></stock-nav>
    ...
    <div class="md:hidden font-benzin-bold text-xl pl-3 mt-1">Сектора</div>
    ...
    <div class="hidden md:grid grid-cols-12 h-5/6">
        ...
        <div class="md:col-span-12 lg:col-span-10 pb-10">
            <div class="shadow-brand rounded-md px-10 pb-5">
                ...
                <router-link :to="{ name: 'Sector', params: { id: s.id }}" v-for="(s, index) in sectors" :key="index" class="grid grid-cols-4 border-b border-light-purple py-3">
                    <div class="flex space-x-2 items-center transition duration-300 transform hover:scale-105">
                        <div class="w-4 h-4 rounded-full" :style="`background-color: ${s.context.color}`"></div>
                        <div>{{ s.name }}</div>
                    </div>
                    ...
                    <p class="transition duration-300 transform hover:scale-105">{{s.keyFactor}}</p>
