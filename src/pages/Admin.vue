<template>
    ...
    <div class="h-screen font-ttnorms p-5 sm:p-10 lg:px-32 mx-auto">
        <div class="rounded-lg p-5 h-5/6 lg:p-10 w-full overflow-scroll shadow-brand">
            ...
            <router-link to="/admin/newStock" class="text-md font-benzin-semibold text-gray-400 hover">+ Добавить акцию</router-link>
        </div>
    </div>
</template>
...
export default defineComponent({
  components: { StockNav },
    setup() {
        ...
        const { stocks, stock, getStocks, addStock, updateStock, deleteStock } = stockFuncs()
        getStocks()
        ...
        return { router, stocks, stock, getStocks, updateStock, deleteStock, addStock, logout, stockParams }
    },
})
</script>
